...
# read the posted values from the UI
    ...
    if not (_name and _email and _password):
        return json.dumps({'message':'User could not be created!'})
    ...
    try:
        if session.get('user'):
            ...
            wish.append({'Id':result[0][0],'Title':result[0][1],'Description':result[0][2],
                        'FilePath':result[0][3],'Private':result[0][4],'Done':result[0][5]})
...
@app.route('/displayComic')
def displayComic():
    return render_template('displayComic.html')

@app.route('/addUpdateLike',methods=['POST', 'GET'])
def addUpdateLike():
    try:
        if session.get('user'):
